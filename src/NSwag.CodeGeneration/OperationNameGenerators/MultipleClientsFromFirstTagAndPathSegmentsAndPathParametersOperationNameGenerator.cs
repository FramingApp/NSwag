...
namespace NSwag.CodeGeneration.OperationNameGenerators
{
    ...
    public class MultipleClientsFromFirstTagAndPathSegmentsAndPathParametersOperationNameGenerator : IOperationNameGenerator
    {
        ...
        internal static string ConvertPathToName(string path)
        {
            ...
            string EndPerameter = path
                .Split('/')
                .Where(p => p.Contains("{") && !string.IsNullOrWhiteSpace(p))
                .Reverse()
                .FirstOrDefault();

            if (EndPerameter != null) {
                EndPerameter = EndPerameter.Trim('{', '}');
                EndPerameter = ConversionUtilities.ConvertToUpperCamelCase(EndPerameter, false);
                EndPerameter = "By" + EndPerameter;

                MainPath += EndPerameter;
            }
            return MainPath;
        }
    }
